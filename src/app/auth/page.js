  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 420, background: 'var(--surface)', border: '1px solid var(--border2)', padding: '48px 40px' }}>
        ...
        {/* Logo */}
        <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 22, fontWeight: 800, color: 'var(--green)', marginBottom: 4 }}>
          Algaeo.io
        </div>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', marginBottom: 28 }}>
          {['signin','signup'].map((t, i) => (
            <button key={t} onClick={() => { setTab(t); setError(''); }} style={{
              ...
              background: 'none', border: 'none', borderBottom: tab === t ? '2px solid var(--green)' : '2px solid transparent',
        ...
        {error && (
          <div style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', color: '#f87171', fontSize: 12, padding: '10px 14px', marginBottom: 16 }}>
            {error}
          </div>
        )}
        ...
        {tab === 'signin' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <button className="btn-primary" onClick={handleSignIn} disabled={loading} style={{ marginTop: 8 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', textAlign: 'center' }}>
              No account?{' '}
              <span style={{ color: 'var(--green)', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => setTab('signup')}>Create one</span>
            </div>
          </div>
        )}
        ...
        {tab === 'signup' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <Field label="Primary Crop">
                <select className="input-base" value={suCrop} onChange={e => setSuCrop(e.target.value)} style={{ background: 'var(--bg)' }}>
                  {['Corn','Soybeans','Wheat','Tomatoes','Berries','Peanuts','Pasture','Miscanthus'].map(c => <option key={c}>{c}</option>)}
                </select>
              </Field>
            </div>
            ...
            <button className="btn-primary" onClick={handleSignUp} disabled={loading} style={{ marginTop: 8 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', textAlign: 'center' }}>
              ...
              <span style={{ color: 'var(--green)', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => setTab('signin')}>Sign in</span>
            </div>
          </div>
        )}
...
function GoogleBtn({ onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      fontFamily: 'DM Mono, monospace', fontSize: 12, padding: 11, cursor: 'pointer',
