...
export default function SoilSamplesPage() {
  ...
  const [view, setView] = useState('map'); // 'map' | 'list' | 'add'
  ...
  const getLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        alert(`Could not get location: ${err.message}. Make sure location access is enabled for your browser.`);
        setLocating(false);
      },
  ...
  const saveSample = async () => {
    if (!form.lat || !form.lng) { alert('Please capture your GPS location first.'); return; }
    if (!userId) return;
    setSaving(true);
    ...
    setForm(p => ({ ...p, lat: null, lng: null, accuracy: null, fieldName: '', notes: '' }));
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '20px', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
          <div>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 700, color: 'var(--text)' }}>Soil Sample Map</div>
            ...
            {selectedSample && (
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: 20 }}>
                ...
                {selectedSample.photo_url && (
                  <img src={selectedSample.photo_url} style={{ width: '100%', maxHeight: 200, objectFit: 'cover', marginBottom: 16, border: '1px solid var(--border)' }} alt="soil sample" />
                )}
        ...
        {view === 'add' && (
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: 24 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 24 }}>Stand at your sample location and tap "Capture GPS Location" first.</div>

            {/* GPS capture — primary action */}
            <div style={{ marginBottom: 24 }}>
              <button onClick={getLocation} disabled={locating} style={{
                width: '100%', padding: '16px', fontSize: 14, fontFamily: 'Syne, sans-serif', fontWeight: 700,
                ...
                cursor: locating ? 'wait' : 'pointer', letterSpacing: '0.04em',
              }}>
                {locating ? '📡 Getting location...' : form.lat ? `📍 ${form.lat.toFixed(5)}, ${form.lng.toFixed(5)} (±${form.accuracy}m) — Tap to reacquire` : '📍 Capture GPS Location'}
              </button>
              {form.accuracy && form.accuracy > 20 && (
                <div style={{ fontSize: 10, color: 'var(--amber)', marginTop: 6 }}>⚠ Accuracy is {form.accuracy}m — move to open sky for better signal</div>
              )}
              {form.accuracy && form.accuracy <= 20 && (
                <div style={{ fontSize: 10, color: 'var(--green)', marginTop: 6 }}>✓ Good accuracy — {form.accuracy}m</div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <F label="Soil Photo">
                <label style={{ cursor: 'pointer', display: 'block' }}>
                  <div style={{ border: `2px dashed ${photoPreview ? 'var(--green-muted)' : 'var(--border2)'}`, background: photoPreview ? 'var(--green-glow)' : 'var(--bg)', height: 140, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                    {photoPreview
                      ? <img src={photoPreview} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
                      : <div style={{ textAlign: 'center', fontSize: 12, color: 'var(--text-muted)' }}>📷 Tap to take photo or upload</div>
                    }
                  </div>
              ...
              <F label="Notes"><textarea className="input-base" value={form.notes} onChange={e => setF('notes', e.target.value)} placeholder="Sample conditions, color, texture, smell..." style={{ resize: 'vertical', minHeight: 72 }} /></F>

              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                <button className="btn-primary" onClick={saveSample} disabled={saving || !form.lat} style={{ flex: 1 }}>
                  {saving ? 'Saving...' : !form.lat ? 'Capture GPS First' : 'Save Sample →'}
                </button>
